...
"""
📊 Teaser Figure Generator with Real Model Inference (S3DIS Version)
"""
...
TARGET_CLASS_OPTIONS = {
    'chair': 7,     # S3DIS class id for chair
    'table': 8,     # S3DIS class id for table
    'sofa': 10,     # S3DIS class id for sofa
    'bookcase': 9,  # S3DIS class id for bookcase
}
...
COLORS = {
    ...
    'bookcase': np.array([0.55, 0.35, 0.20]),

    'wall': np.array([0.60, 0.60, 0.60]),
    'floor': np.array([0.50, 0.50, 0.50]),
    'ceiling': np.array([0.70, 0.70, 0.70]),
...
def create_teaser_figure(sample, output_path, dpi=300):
    ...
    print(f"     (S3DIS validation set - Area 5)")
    ...
    TEXTURE_COLORS = {
        'ceiling': np.array([0.70, 0.70, 0.72]),
        'floor': np.array([0.65, 0.55, 0.45]),
        'wall': np.array([0.78, 0.80, 0.82]),
        'beam': np.array([0.55, 0.48, 0.40]),
        'column': np.array([0.60, 0.55, 0.50]),
        'window': np.array([0.72, 0.82, 0.92]),
        'door': np.array([0.58, 0.45, 0.35]),
        'table': np.array([0.68, 0.58, 0.48]),
        'clutter': np.array([0.62, 0.62, 0.62]),
    }
    ...
    SEMANTIC_COLORS = {
        0: np.array([0.70, 0.70, 0.72]),   # ceiling
        1: np.array([0.65, 0.55, 0.45]),   # floor
        2: np.array([0.78, 0.80, 0.82]),   # wall
        3: np.array([0.55, 0.48, 0.40]),   # beam
        4: np.array([0.60, 0.55, 0.50]),   # column
        5: np.array([0.72, 0.82, 0.92]),   # window
        6: np.array([0.58, 0.45, 0.35]),   # door
        7: np.array([0.35, 0.15, 0.55]),
        8: np.array([0.68, 0.58, 0.48]),   # table
        9: np.array([0.52, 0.45, 0.38]),   # bookcase
        10: np.array([0.35, 0.15, 0.55]),
        11: np.array([0.85, 0.85, 0.78]),  # board
        12: np.array([0.62, 0.62, 0.62]),  # clutter
    }
    ...
    with open(info_path, 'w') as f:
        ...
        f.write(f"Dataset: S3DIS (validation set - Area 5)\n")
        f.write(
...
def main():
    ...
    parser.add_argument('--output', type=str,
                        default='figures/teaser_s3dis.png')
    parser.add_argument('--cfg', type=str,
                        default='cfgs/s3dis/pointnext-xl.yaml')
    parser.add_argument('--baseline-ckpt', type=str,
                        default='log/s3dis/train-xl-origin/checkpoint/s3dis-train-pointnext-xl-ngpus1-20251227-120137-7K9Wv2GjdqiLSa46w5m6nG_ckpt_best.pth')
    parser.add_argument('--ours-ckpt', type=str,
                        default='log/s3dis/train-xl-ours/checkpoint/s3dis-train-pointnext-xl-ngpus1-20260101-100327-Yz92P6UJGg7iiMq9JLspEL_ckpt_best.pth')
    parser.add_argument('--gpu', type=int, default=0)
    parser.add_argument('--max-search', type=int, default=100)
    parser.add_argument('--dpi', type=int, default=300)
    parser.add_argument('--target', type=str, default='sofa',
                        choices=['chair', 'table', 'sofa', 'bookcase'],
                        help='Target class to attack (chair, table, sofa, or bookcase)')
    args = parser.parse_args()

    print("\n" + "=" * 70)
    print("📊 TEASER FIGURE GENERATOR (S3DIS Version - Same as Paper)")
    print("=" * 70)
