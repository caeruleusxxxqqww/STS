...
class RevBackProp(torch.autograd.Function):
    ...
    def forward(ctx, x, blocks, pos_embed=None, alpha=0., lambd=1., num_cached=0):
        ...
        if len(buffer_layers) == 0:
            all_tensors = [x.detach(), pos_embed.detach()
                           if pos_embed is not None else pos_embed]
        else:
            intermediate = [torch.LongTensor(
                buffer_layers), *intermediate]  # for debug
            all_tensors = [x.detach(), pos_embed.detach(
            ) if pos_embed is not None else pos_embed, *intermediate]  # for debug
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0.,
        ...
        Fm = Block_inv_F(dim=dim, num_heads=num_heads, qkv_bias=qkv_bias, drop=drop,
                         attn_drop=attn_drop, norm_args=norm_args, drop_path=drop_path)
        Gm = Block_inv_G(dim=dim,  mlp_ratio=mlp_ratio, drop=drop,
                         act_args=act_args, norm_args=norm_args, drop_path=drop_path)
...
class InvPointViT(nn.Module):
    ...
    def __init__(self,
        ...
        if self.patch_embed.out_channels != self.embed_dim:
            self.proj = nn.Linear(
                self.patch_embed.out_channels, self.embed_dim)
        else:
    ...
    def get_alpha_lambd_old(self, epoch=-1, step=-1):
        if self.stop_epoch > 0:
            ...
            step = step if epoch < self.stop_epoch else (
                self.stop_epoch-1) * 2000 / 8
        ...
        if self.option != -1:
            ...
            if epoch < self.init_epoch:  # Inference
                ...
                # Option 0: linear, interleave alpha and lambda
                    ...
                    alpha = round(min(max(-round(self.freq/(end_epoch-start_epoch), 12) * math.ceil(
                        (epoch-start_epoch-1-self.freq//2) / self.freq) + 1, 0), 1.), 4)
                # self.option 1: linear alpha = (1/(A+1-N))*x + N/(N-A-1), lambd = 1 - alpha + 0.01
                elif self.option == 1:
                    alpha = round(1/(start_epoch+1-end_epoch) *
                                  epoch + end_epoch/(end_epoch-start_epoch-1), 2)
                # self.option 2: logarithmic
                    ...
                    alpha = round(
                        max(min(a * math.log(epoch, base) + c, 1.), 0), 2)
            ...
            if epoch < self.init_epoch:  # Inference
                ...
                if self.option == 6:
                    lambd = -1 * round(max(min(self.freq/(end_epoch-start_epoch) * math.ceil(
                        (epoch-start_epoch-1) / self.freq) + self.init_lambd, 1.), self.init_lambd), 4)
                elif self.option == 0 or self.option == 5:
                    lambd = round(max(min(self.freq/(end_epoch-start_epoch) * math.ceil(
                        (epoch-start_epoch-1) / self.freq) + self.init_lambd, 1.), self.init_lambd), 4)
                else:
    ...
    def lambd_alpha_options(self, option=0, current=-1, start=0, end=10, freq=10., freeze_ratio=0.6):
        ...
        # breakpoint()
        if option == 0:
            lambd = round(max(min((freq/(end-start) * math.ceil((current -
                          start-1) / freq) + lambd_min), lambd_max), lambd_min), 4)
            alpha = round(min(max(-round(freq/(end-start), 12) * math.ceil(
                (current-start-1-freq//2) / freq) + 1, alpha_min), alpha_max), 4)
        # elif option == -1:
        ...
        #     alpha = 1.0
            ...
            if current < end_half:
                ...
                lambd = round(max(min(freq/(end-end_half) * math.ceil((current -
                              end_half-1) / freq) + lambd_min, lambd_max), lambd_min), 4)
            alpha = round(min(max(-round(freq/(end_half-start), 12) * math.ceil(
                (current-start-1-freq//2) / freq) + 1, alpha_min), alpha_max), 4)
        elif option == 6:
            freq = int(freq/2)
            end_half = start + int((end - start) / 2)

            lambd = round(max(min((freq/(end_half-start) * math.ceil((current -
                          start-1) / freq) + lambd_min), lambd_max), lambd_min), 4)
            if current < end_half:
                alpha = alpha_max
            else:
                alpha = round(min(max(-round(freq/(end-end_half), 12) * math.ceil(
                    (current-end_half-1-freq//2) / freq) + 1, alpha_min), alpha_max), 4)
        return lambd, alpha
    ...
    def forward(self, p, x=None, epoch=-1, step=-1):
        if hasattr(p, 'keys'):
            p, x, epoch, step = p['pos'], p['x'] if 'x' in p.keys(
            ) else None, p['epoch'] if 'epoch' in p.keys() else -1, p['iter'] if 'iter' in p.keys() else -1
        if x is None:
        ...
        with torch.set_grad_enabled(epoch >= self.start_epoch):
            ...
            if self.add_pos_each_block:
                if use_inv:
                    x = self._use_inv(x, pos_embed, alpha,
                                      lambd, num_cached=self.num_cached)
                else:
    ...
    def _use_inv(self, x, pos_embed=None, alpha=0., lambd=1., num_cached=0):
        ...
        if self.use_customized_backprop:
            x = RevBackProp.apply(x, self.blocks, pos_embed,
                                  alpha, lambd, num_cached)
        else:
            for block in self.blocks:
                x = block(x, pos_embed)
        C = int(x.shape[-1] / 2)
    ...
    def forward_cls_feat(self, p, x=None):  # p: p, x: features
        ...
        for token_type in self.global_feat:
            if 'cls' in token_type:
                ...
                cls_feats.append(
                    torch.max(token_features, dim=1, keepdim=False)[0])
            elif token_type in ['avg', 'mean']:
                cls_feats.append(torch.mean(
                    token_features, dim=1, keepdim=False))
...
class InvPointViTDecoder(nn.Module):
    ...
    def __init__(self,
        ...
        if progressive_input:
            skip_dim = [self.in_channels//2 **
                        i for i in range(n_decoder_stages-1, 0, -1)]
        else:
    ...
    def forward(self, p, f):
        ...
        if len(p) != (self.n_decoder_stages + 1):
            for i in range(self.n_decoder_stages - 1):
                ...
                new_p = torch.gather(
                    pos, 1, idx.unsqueeze(-1).expand(-1, -1, 3))
                p.insert(1, new_p)
                f.insert(1, None)
        cls_token = f[-1][:, :, 0:1]
        ...
        if self.global_feat is not None:
            ...
            for token_type in self.global_feat:
                if 'cls' in token_type:
                    ...
                    global_feats.append(
                        torch.max(f_out, dim=2, keepdim=True)[0])
                elif token_type in ['avg', 'mean']:
                    global_feats.append(torch.mean(f_out, dim=2, keepdim=True))
            global_feats = torch.cat(
                global_feats, dim=1).expand(-1, -1, f_out.shape[-1])
...
class InvPointViTPartDecoder(nn.Module):
    ...
    def forward(self, p, f, cls_label):
        ...
        if self.cls_map == 'pointnet2':
            cls_one_hot = torch.zeros(
                (B, self.num_classes), device=p[0].device)
