...
try:
    from openpoints.loss.custom_innovations import apply_semantic_texture_swapping
    print("[BaseSeg] Successfully imported STS augmentation module.")
except ImportError as e:
    apply_semantic_texture_swapping = None
    print(f"[BaseSeg] Warning: Failed to import STS augmentation module: {e}")
# ------------------------
...
class BaseSeg(nn.Module):
    def __init__(self,
        ...
        if decoder_args is not None:
            ...
            decoder_args_merged_with_encoder.encoder_channel_list = self.encoder.channel_list if hasattr(
                self.encoder, 'channel_list') else None
            self.decoder = build_model_from_cfg(
    ...
    def forward(self, data, apply_sts=False):
        """
        Forward pass.

        Args:
            data: Input data dict
            apply_sts: Whether to apply STS augmentation (for consistency loss)
        """
        sts_info = {'sts_applied': False, 'sts_mask': None}
        if apply_sts and self.training and apply_semantic_texture_swapping is not None:
            data = apply_semantic_texture_swapping(data, return_info=True)
            sts_info['sts_applied'] = data.get('sts_applied', False)
            sts_info['sts_mask'] = data.get('sts_mask', None)
        ...
        #     original_rgb = data.x[:, :3]
        # else:
        #     original_rgb = data.pos

        # original_batch = data.get('batch', None)
        ...
        if original_pos.dim() == 3:
            B, N, _ = original_pos.shape
            original_pos = original_pos.reshape(-1, original_pos.shape[-1])

            if data.x.dim() == 3:
                original_rgb = data.x.transpose(1, 2)[:, :, :3].reshape(-1, 3)
            else:
                original_rgb = data.x[:, :3]

            original_batch = torch.arange(
                B, device=original_pos.device).repeat_interleave(N)
        else:
            original_rgb = data.x[:, :3]
        ...
        # -----------------------------------------------------------------
        features_for_loss = f
        if features_for_loss.dim() == 3:
            features_for_loss = features_for_loss.transpose(
                1, 2).contiguous().view(-1, features_for_loss.shape[1])
        ...
        if self.training:
            return {
                "logits": logits,
                "features": features_for_loss,
                "pos": original_pos,
                "rgb": original_rgb,
                "batch": original_batch,
                "sts_applied": sts_info['sts_applied'],
                "sts_mask": sts_info['sts_mask']
            }
        else:
...
class BasePartSeg(BaseSeg):
    def __init__(self, encoder_args=None, decoder_args=None, cls_args=None, **kwargs):
        logging.warning("[BasePartSeg] SCJ and TSI modules not implemented in this model (incompatible forward signature).")
        kwargs.pop('tsi_loss_args', None)
...
class VariableSeg(BaseSeg):
    def __init__(self,
                 ...
                 tsi_loss_args=None,
                 **kwargs):
        super().__init__(encoder_args, decoder_args,
                         cls_args, tsi_loss_args=tsi_loss_args, **kwargs)
        if kwargs:
            logging.warning(
    ...
    def forward(self, end_points):
        if self.global_feat is not None:
            ...
            for feat_type in self.global_feat:
                if 'max' in feat_type:
                    global_feats.append(
                        torch.max(end_points, dim=-1, keepdim=True)[0])
                elif feat_type in ['avg', 'mean']:
                    global_feats.append(torch.mean(
                        end_points, dim=-1, keepdim=True))
            global_feats = torch.cat(
                global_feats, dim=1).expand(-1, -1, end_points.shape[-1])
    ...
    def __init__(self,
        ...
        for i in range(shape_classes):
            ...
            for j in range(len(mlps) - 2):
                ...
                head.append(
                    nn.Conv1d(mlps[-2], num_parts[i], kernel_size=1, bias=True))
