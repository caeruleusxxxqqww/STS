...
class BallQuery(Function):
    ...
    def forward(ctx, radius: float, nsample: int, xyz: torch.Tensor, new_xyz: torch.Tensor) -> torch.Tensor:
        ...
        idx = torch.cuda.IntTensor(
            B, npoint, nsample, device=xyz.device).zero_()
...
class QueryAndGroup(nn.Module):
    def __init__(self, radius: float, nsample: int,
        ...
        assert self.normalize_dp + self.normalize_by_std + \
            self.normalize_by_allstd < 2   # only nomalize by one method
    ...
    def forward(self, query_xyz: torch.Tensor, support_xyz: torch.Tensor, features: torch.Tensor = None) -> Tuple[
        ...
        if self.relative_xyz:
            grouped_xyz = grouped_xyz - \
                query_xyz.transpose(1, 2).unsqueeze(-1)  # relative position
...
class KNNGroup(nn.Module):
    ...
    def forward(self, query_xyz: torch.Tensor, support_xyz: torch.Tensor, features: torch.Tensor = None) -> Tuple[
        ...
        if self.normalize_dp:
            grouped_xyz /= torch.amax(torch.sqrt(torch.sum(grouped_xyz **
                                      2, dim=1)), dim=(1, 2)).view(-1, 1, 1, 1)
