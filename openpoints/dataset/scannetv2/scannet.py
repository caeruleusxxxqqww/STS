...
class ScanNet(Dataset):
    ...
    def __init__(self,
        ...
        logging.info("Totally {} ScanNet samples in {} set.".format(
    ...
    def __getitem__(self, idx):
        ...
        if self.presample:
            coord, feat, label = np.split(
                self.data[data_idx], [3, 6], axis=1)
        else:
        ...
        label = label.astype(np.int64).squeeze() if hasattr(
            label, 'squeeze') else label.astype(np.int64)
        data = {'pos': coord.astype(np.float32),
                'x': feat.astype(np.float32), 'y': label}
        """debug 
        ...
        if 'heights' not in data.keys():
            data['heights'] = data['pos'][:, self.gravity_dim:self.gravity_dim +
                                          1] - data['pos'][:, self.gravity_dim:self.gravity_dim+1].min()
        return data
