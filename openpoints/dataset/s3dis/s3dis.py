    ...
    def __init__(self,
        ...
        if split == 'train':
            ...
            if train_ratio < 1.0:
                ...
                num_keep = max(1, int(len(self.data_list) * train_ratio))
                indices = rng.choice(len(self.data_list),
                                     num_keep, replace=False)
                indices.sort()
                self.data_list = [self.data_list[i] for i in indices]
                logging.info(f"[Few-shot] Using {num_keep}/{len(self.data_list)} "
                             f"training rooms ({train_ratio*100:.0f}%)")
        else:
            self.data_list = [
