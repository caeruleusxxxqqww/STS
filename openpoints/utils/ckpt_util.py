...
def load_checkpoint(model, pretrained_path, module=None):
    ...
    if skipped_keys:
        logging.info(f"[load_checkpoint] Skipped {len(skipped_keys)} keys due to shape mismatch:")
    ...
    if hasattr(model, 'module'):
        incompatible = model.module.load_state_dict(filtered_ckpt, strict=False)
    else:
